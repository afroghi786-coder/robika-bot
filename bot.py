# ============================================================
# 🤖 ربات فروشگاهی - نسخه نهایی با حل قطعی شماره فاکتور و کد مشتری
# ============================================================
...
def get_column_index_by_title(records, title_variants):
    """پیدا کردن ایندکس ستون با استفاده از لیستی از عنوان‌های احتمالی"""
    if len(records) == 0:
        return None
    header = records[0]
...
def get_last_invoice_number_from_sheet():
    ...
    try:
        ...
        if len(records) < 2:
            logging.info("ℹ️ شیت فروش خالی است، شمارنده از ۰ شروع می‌شود.")
            return 0
        
        # جستجوی ستون شماره_فاکتور با چندین الگوی احتمالی
        ...
        if col_invoice is None:
            logging.error("❌ ستون شماره_فاکتور پیدا نشد! شمارنده از ۰ شروع می‌شود.")
            return 0
            ...
            if len(row) > col_invoice:
                ...
                if val.startswith("M_"):
                    try:
                        ...
                        pass
        logging.info(f"📊 بزرگترین شماره فاکتور موجود: {max_num}")
        return max_num
                ...
                if phone and code:
                    ...
                    if code.startswith("MO_"):
                        try:
                            ...
                            pass
        logging.info(f"📊 {len(phone_to_code)} شماره تماس با کد مشتری پیدا شد. بزرگترین کد: {max_code}")
...
if last_invoice > data.get("invoice_counter", 0):
    ...
    save_data(data)
    logging.info(f"🔄 شمارنده فاکتور به‌روزرسانی شد: {data['invoice_counter']}")

# ✅ هماهنگ‌سازی کد مشتری با شیت فروش
...
if max_code > data.get("customer_counter", 3000):
    ...
    save_data(data)
    logging.info(f"🔄 شمارنده کد مشتری به‌روزرسانی شد: {data['customer_counter']}")
...
def generate_invoice_number():
    ...
    save_data(data)
    now = datetime.now()
    new_invoice = f"M_{now.strftime('%Y%m%d')}{invoice_counter:04d}"
    logging.info(f"📄 شماره فاکتور جدید: {new_invoice}")
    return new_invoice
...
def get_or_create_customer_code(phone):
    ...
    # ۱. بررسی دیکشنری محلی
    if phone in customer_codes:
        logging.info(f"🔁 کد مشتری از دیکشنری محلی: {customer_codes[phone]}")
        return customer_codes[phone]
    
    # ۲. خواندن از شیت فروش
    phone_to_code, max_code = get_existing_customer_data()
    if phone in phone_to_code:
        ...
        save_data(data)
        logging.info(f"🔁 کد مشتری از شیت فروش: {code}")
        return code
    ...
    data["customer_codes"] = customer_codes
    save_data(data)
    logging.info(f"🆕 کد مشتری جدید: {code}")
    return code
...
def ثبت_سفارش_در_شیت(customer, items, total, invoice_number, customer_code):
    try:
        ...
        logging.info(f"📤 ارسال به Webhook: {json.dumps(payload, ensure_ascii=False)}")
        response = requests.post(WEBHOOK_URL, json=payload, timeout=10)
        if response.status_code == 200:
            result = response.json()
            if result.get("status") == "success":
                logging.info(f"✅ سفارش {invoice_number} در گوگل‌شیت ثبت شد.")
                return True
        logging.error(f"❌ خطا در ثبت سفارش: {response.text}")
        return False
    except Exception as e:
        logging.error(f"❌ خطا در ثبت سفارش: {e}")
        return False
...
def به‌روزرسانی_واریزی_در_شیت(invoice_number, payment_amount, account_number="", account_holder=""):
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            if result.get("status") == "success":
                logging.info(f"✅ واریزی {invoice_number} ثبت شد.")
...
# ============================================================
# 🖼️ تولید فاکتور (کوتاه‌شده برای اختصار)
# ============================================================
...
# ============================================================
# 🤖 ساخت ربات و هندلرها (همان کد قبلی)
# ============================================================
